...
### 1. Die Symmetrische Paarungs-Lüge (`mtho_state_vector.py`)
**Der Ist-Zustand:**
```python
MTHO_PAIRINGS = {
...
### 2. Der statische "Wuji"-Tod (`mtho_state_vector.py`)
**Der Ist-Zustand:**
```python
WUJI = MTHOStateVector(x_car_cdr=0.5, y_gravitation=0.0, z_widerstand=0.5, w_takt=0)
```
**Die Illusion:** Der Grundzustand `WUJI` ist exakt mittig ausbalanciert ($0.5$). 
**Die Realität:** Ein System auf exakt $0.5$ hat keinen Antrieb für den ersten Takt. Wenn Symmetriebruch ($0.49/0.51$) und das baryonische Delta ($0.049$) der Motor der Kausalität sind, darf `WUJI` niemals exakt $0.5$ sein. Es muss zwingend mit dem primordialen Offset (z.B. $0.49$) initialisiert werden, sonst erzeugt es ein totes, unendliches Gleichgewicht (keine Zeit).
...
### 4. Chargaff-Balance (DNA) ohne Reibungsverlust (`mtho_knowledge.py`)
**Der Ist-Zustand:**
...
1. **Asymmetrisches Wuji:** `mtho_state_vector.py` umschreiben. `x=0.49` / `z=0.51`. Eliminierung aller `0.5`-Deadlocks.
2. **Korrektur der MTHO-Paarungen:** Abbildung der orthogonalen Asymmetrie (O↔T Motor, H↔M Rückgrat) anstelle der plumpen 1:1-Dict-Maps.
