...
void JarvisSystem::updateClock()
{
    ...
    const QLocale de(QStringLiteral("de_DE"));
    m_currentTime = de.toString(now.time(), QLocale::ShortFormat);
    if (m_currentTime.length() < 5) {
        m_currentTime = now.toString(QStringLiteral("HH:mm:ss"));
    }
    m_currentDate = de.toString(now.date(), QLocale::LongFormat);
