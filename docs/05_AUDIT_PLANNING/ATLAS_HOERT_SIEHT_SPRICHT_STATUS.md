...
### ATLAS hört – **[OK]** Pipeline via Dreadnought

- **Dreadnought:** `dreadnought_listen.py` nimmt Audio auf (oder Dummy-Fallback bei fehlender Hardware) und sendet es als Event an OC Brain.
- **Beweis:** `proof_hoert_sieht_spricht.py` meldet Erfolg beim Senden des Audio-Events an OC Brain.

### ATLAS sieht – **[OK]** MX/Brio via Scout → go2rtc → HA

- **Status:** MX Brio (USB am Scout/Raspi) registriert in go2rtc (`mx_brio`) und in HA (`camera.192_168_178_54`).
- **Pipeline:** USB `/dev/video0` → go2rtc (Scout:1984) → RTSP (Scout:8554) → HA Generic Camera → ATLAS.
...
### ATLAS spricht – **[OK]** TTS auf Google Minis + WhatsApp
