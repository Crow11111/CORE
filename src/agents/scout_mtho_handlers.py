...
async def handle_deep_reasoning(payload: dict) -> dict:
    ...
    try:
        ...
        reply = mtho_llm.invoke_heavy_reasoning(
            "Du bist MTHO. Antworte praezise und analytisch.",
            text
        )
...
async def handle_failover(payload: dict) -> dict:
    ...
    vps_url = (os.getenv("MTHO_VPS_URL") or "").strip().rstrip("/")
    if not vps_url:
        return {"error": "MTHO_VPS_URL nicht konfiguriert", "success": False}
