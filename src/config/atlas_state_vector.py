...
from src.mtho_core import (
    ...
    BARYONIC_LIMIT, LPIS_LEGACY_MAP
)
...
# ---------------------------------------------------------------------------
# Legacy LPIS Layer (DEPRECATED - Compatibility Only)
# ---------------------------------------------------------------------------
# Mappt alte LPIS Keys auf die neuen MTHO Definitionen
LPIS_BASES = {
    "L": {**MTHO_BASES["O"], "name": "Logisch (Legacy)", "dna": "T (Old)"}, # L -> O
    "P": {**MTHO_BASES["M"], "name": "Physikalisch (Legacy)", "dna": "A (Old)"}, # P -> M
    "I": {**MTHO_BASES["T"], "name": "Information (Legacy)", "dna": "C (Old)"}, # I -> T
    "S": {**MTHO_BASES["H"], "name": "Strukturell (Legacy)", "dna": "G (Old)"}, # S -> H
}
...
TETRALOGIE = {
    "AGENCY": {"takt": 3, "mtho": "M", "lpis": "P", "car": "Effizienz", "cdr": "Clean Code"},
    "COUNCIL": {"takt": [1, 4], "mtho": "O", "lpis": "L", "car": "Paranoia", "cdr": "Compliance"},
    "FORGE": {"takt": 2, "mtho": "T", "lpis": "I", "car": "Chaos", "cdr": "Architektur-Spec"},
    "ARCHIVE": {"takt": 4, "mtho": "H", "lpis": "S", "car": "Vektor-Cluster", "cdr": "SQL-Index"},
}
...
SIMULATION_EVIDENCE_STATS = {
    ...
    "indizien": 58,
    "max_aeste": 13,
    "lpis_verteilung": {"L": 19, "P": 13, "I": 13, "S": 13}, # Legacy keys kept for stats compatibility
...
def state_to_embedding_text() -> str:
    ...
    """
    return f"""ATLAS 4D State Vector - Bootloader (MTHO Native)
