...
*   **Windows Firewall (Node Alpha):**
    ```powershell
    New-NetFirewallRule -DisplayName "CORE Inbound SSH" -Direction Inbound -Action Allow -Protocol TCP -LocalPort 22
    New-NetFirewallRule -DisplayName "CORE Ollama API" -Direction Inbound -Action Allow -Protocol TCP -LocalPort 11434
    ```
*   **IPTables (Node Beta):**
    ```bash
