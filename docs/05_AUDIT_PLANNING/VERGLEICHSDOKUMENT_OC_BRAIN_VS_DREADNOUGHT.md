...
# Vergleichsdokument: OMEGA_ATTRACTOR (VPS-ChromaDB) vs. 4D_RESONATOR (MTHO_CORE)/Repo

**Zweck:** Abgleich, was OMEGA_ATTRACTOR in die ChromaDB geschrieben hat vs. was im Repo und lokal (4D_RESONATOR (MTHO_CORE)) definiert ist. Basis für Abstimmung mit User.
...
## 1. Erwartete core_directives (Repo/4D_RESONATOR (MTHO_CORE))
...
**Lokal (4D_RESONATOR (MTHO_CORE)) zusätzlich vorhanden:** ring0_bias_depth_check, ring0_negentropie_check, ring0_konstruktive_dissonanz, ring0_scaffolding, test_probe (ältere Seeds).
...
| Typ | Beschreibung |
|-----|--------------|
| Lokal (4D_RESONATOR (MTHO_CORE)) | Alle 4 Repo-Mindest-IDs vorhanden; 5 zusätzliche Ring-0-/Test-Direktiven (konsistent mit Abschnitt 1). |
...
Lokal sind alle erwarteten core_directives vorhanden; keine Nachziehe nötig. Optional: VPS-Abgleich (SSH-Tunnel + Skript) zur Bestätigung der OMEGA_ATTRACTOR-Befüllung. Ring-0-/Test-Direktiven (ring0_*, test_probe) als 4D_RESONATOR (MTHO_CORE)-spezifisch belassen oder mit User abstimmen, ob auf VPS synchronisiert werden sollen.
