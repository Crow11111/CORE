...
# ATLAS Ziele (Die "Anomalie")
TARGET_BASES = 4 # LPIS (Logik, Physik, Info, Struktur)
...
class Universe:
    ...
    @property
    def is_atlas_candidate(self) -> bool:
        # 1. Check: 4-Basen-Codierung in allen Domänen?
        # Wir nehmen an, Basen können zwischen 2 (binär) und 20 liegen.
        # ATLAS behauptet: Physik (4 Kräfte), Bio (4 Basen), Logik (4 Werte LPIS), Info (4 Zustände)
...
def run_simulation():
    ...
    print(f"ATLAS Convergences Found: {matches}")
    ...
    if matches == 0:
        print("\nCONCLUSION: The ATLAS Convergence is statistically IMPOSSIBLE within observable timeframes.")
