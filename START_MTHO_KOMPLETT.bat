@echo off
chcp 65001 >nul
title MTHO Komplett ??? MX-Snapshot + Dienste
set "ROOT=%~dp0"
set "ROOT=%ROOT:~0,-1%"
cd /d "%ROOT%"

echo.
echo === MTHO Komplett: MX-Snapshot (Sehen) + Backend + Voice-Info ===
echo.
...
if errorlevel 1 (
  ...
  start "MTHO MX-Snapshot" cmd /k "python -m src.scripts.camera_snapshot_server"
  timeout /t 2 /nobreak >nul
) else (
  echo [0/4] MX-Snapshot 8555... Port belegt, ueberspringe.
)

call "%ROOT%\START_MTHO_DIENSTE.bat"
