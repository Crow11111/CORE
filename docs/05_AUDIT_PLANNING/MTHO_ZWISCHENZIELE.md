...
# MTHO Zwischenziele (Orchestrator-vorgegeben)

**Verantwortung:** Orchestrator setzt Ziele, setzt Teams an, verifiziert jeden Beweis selbst.  
**Phase:** Prototyp 0.5–0.6 – erste Schnittstellen funktionsfähig. TEAM baut MTHO; OMEGA_ATTRACTOR einbeziehen (Teilaufgaben, Orchestrierung, Anfragen).
...
| **Z12** | OMEGA_ATTRACTOR in Task-Loop | Konkrete Teilaufgabe an OMEGA_ATTRACTOR senden (z. B. "Priorisiere nächste 3 MTHO-Zwischenziele"); Antwort strukturiert nutzbar. |
| **Z13** | E2E: Event → OMEGA_ATTRACTOR → Antwort → optional TTS | Ein Event löst OC-Antwort aus; Antwort kann an /api/atlas/tts übergeben werden. Verifikation durch Orchestrator. |
| **Z14** | API-Health + OC-Status gebündelt | Ein Endpoint oder Skript liefert Status: Gateway, Event-Ingest, Voice-Roles, letzte Events. Für 4D_RESONATOR (MTHO_CORE)-Dashboard. |
| **Z15** | rat_submissions Roundtrip | OMEGA_ATTRACTOR schreibt in rat_submissions; 4D_RESONATOR (MTHO_CORE) holt per fetch; Inhalt verifiziert. |
...
## Test an 4D_RESONATOR (MTHO_CORE)
