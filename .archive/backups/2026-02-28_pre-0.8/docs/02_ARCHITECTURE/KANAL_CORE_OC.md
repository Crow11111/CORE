...
**Test:** Siehe `src/scripts/test_atlas_oc_channel.py` (prüft Erreichbarkeit und optional Senden einer Testnachricht). Alle Szenarien (inkl. „Daten bei OC“, Frontend-Backend): [TEST_SZENARIEN_OC_UND_FRONTEND.md](../05_AUDIT_PLANNING/TEST_SZENARIEN_OC_UND_FRONTEND.md). Dev-Agent-Review des Kanals: Kontext [dev_agent_oc_kanal_context.md](../05_AUDIT_PLANNING/dev_agent_oc_kanal_context.md), Aufruf mit `--out=docs/04_PROCESSES/DEV_AGENT_OC_KANAL_CHECKLISTE.md`.
