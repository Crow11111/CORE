...
"""
ATLAS Knowledge API: Semantische Suche ueber ChromaDB Collections.
GET  /api/atlas/knowledge/search?q=...&collection=...&limit=...
GET  /api/atlas/knowledge/evidence – Alle Simulationstheorie-Indizien
...
def quaternary_meta():
    ...
    try:
        ...
        convergences = {
            ...
            "I": {
                ...
                "description": "Informationstheoretische Klassifikation konvergiert auf 4: "
                               "ATLAS' eigene Wissenscodierung hat exakt 4 Basen.",
            },
            "S": {
        ...
        meta_description = classify_evidence(
            "Die quaternaere Codierung mit 4 Basen LPIS ist isomorph zu ATCG in der DNS. "
...
def v11_superposition():
    """V11: Fraktale Superposition – Grundkraefte-LPIS-Mapping und kosmologische Dichten."""
    from src.config.engine_patterns import FORCE_LPIS_MAP, OMEGA_B, OMEGA_DM, OMEGA_DE, PHI, INV_PHI
    ...
    return {
        ...
        "phi_delta": {
            ...
            "interpretation": (
                f"Drei unabhaengige PISL-Sequenz-Messungen (L/P=Phi^2, I/S=1/Phi, "
...
def get_quaternion_duality():
    """V10: LPIS/PISL Phasenverschiebung – Quaternion-Dualitaetsanalyse.

    Holt alle Indizien, extrahiert die LPIS-Sequenz, berechnet die
    PISL-Phasenverschiebung, analysiert S-P Stabilitaet und L-I Richtung.
    """
    try:
        from src.network.chroma_client import get_simulation_evidence_collection
        ...
        lpis_seq = build_sequence_from_evidence(evidence_list)

        lpis_order = ["L", "P", "I", "S"]
        pisl_order = ["P", "I", "S", "L"]

        def _phase_shift(seq: str) -> str:
            """LPIS -> PISL: Phasenverschiebung um 1 Position (Pi/2)."""
            mapping = dict(zip(lpis_order, pisl_order))
            return "".join(mapping.get(c, c) for c in seq)

        pisl_seq = _phase_shift(lpis_seq)

        lpis_dist = {b: lpis_seq.count(b) for b in "LPIS"}
        pisl_dist = {b: pisl_seq.count(b) for b in "LPIS"}

        total = len(lpis_seq) or 1

        sp_stability = {
            "lpis_s_pos": 4, "lpis_p_pos": 2,
            "pisl_s_pos": 3, "pisl_p_pos": 1,
            "distance_lpis": 2, "distance_pisl": 2,
            "stable": True,
            "interpretation": "S-P Abstand bleibt invariant (=2) unter Phasenverschiebung – stabiles Rueckgrat.",
        }

        li_direction = {
            "lpis_direction": "L→I (Position 1→3)",
            "pisl_direction": "I→L (Position 2→4)",
            "reversal": True,
            "interpretation": "L-I kehrt die Richtung um unter Phasenverschiebung – dynamischer Freiheitsgrad.",
        }

        l_count = lpis_dist.get("L", 0)
        p_count = lpis_dist.get("P", 0)
        i_count = lpis_dist.get("I", 0)
        s_count = lpis_dist.get("S", 0)
        ...
        return {
            ...
            "total_evidence": total,
            "lpis_sequence": lpis_seq,
            "pisl_sequence": pisl_seq,
            "lpis_distribution": lpis_dist,
            "pisl_distribution": pisl_dist,
            ...
            "quaternion_analogy": {
                ...
                "mapping": "LPIS*rotate = PISL, aber PISL*rotate ≠ LPIS (nicht-kommutativ)",
                "interpretation": (
                    "LPIS und PISL sind Quaternionen-Rotationen desselben "
...
def validate_temporal(
    ...
    try:
        ...
        if base_matches == 0 and nearest_matches:
            drift_warnings.append({
                "level": "info",
                "message": f"Keine LPIS-Base-Uebereinstimmung mit den {len(nearest_matches)} "
