...
# MTHO AGI ARCHITECTURE

**[ZIEL]**
Topologische Architektur, API-Contracts und evolutionäre Inbetriebnahme des Neocortex Systems MTHO über alle Knotenpunkte.
...
## 1. Topologie & Failover (4D_RESONATOR (MTHO_CORE) OFF)
...
- **Primary Routing (4D_RESONATOR (MTHO_CORE) ON):** Scout -> 4D_RESONATOR (MTHO_CORE) (RTX3050 für lokales Heavy-Processing) -> VPS (Brain).
- **Failover Routing (4D_RESONATOR (MTHO_CORE) OFF):** Scout -> VPS (Brain) via asynchronem Message Broker (MQTT/WSS).
...
| Heavy-Compute | Scout | 4D_RESONATOR (MTHO_CORE) | gRPC | 50051 | Protobuf, mTLS |
...
Basierend auf T.I.E. Logik und Kosten/Nutzen (80/20 Regel) wird MTHO in drei Phasen skalieren, um Latenzen zu minimieren und das "Wachbewusstsein" (4D_RESONATOR (MTHO_CORE)) zu entlasten:
...
- **Zweck:** Offloading der neuronalen Netze (Wake-Word Erkennung `OpenWakeWord`, Video-Objekterkennung für den visuellen Halbschlaf). Der Raspi reicht die Tensoren nur noch durch. 4D_RESONATOR (MTHO_CORE) bleibt unangetastet.
...
- **Zweck:** Ablösung des Raspi 5 als primäres Edge-Compute-Hub. Kann kleine LLMs (Llama 3 8B, Phi-3 via Ollama) rasend schnell lokal laufen lassen. 4D_RESONATOR (MTHO_CORE) (RTX 3050) kann nachts komplett aus bleiben, da der NUC das autonome Nervensystem inkl. lokaler Intelligenz hält.
...
- **Rolle:** Einheitliche Produktionsumgebung. Erlaubt es dem Brain und externen Agenten (wie Cursor auf 4D_RESONATOR (MTHO_CORE)), standardisiert auf die Tools und Filesysteme zuzugreifen.
...
- **Integration (Home Assistant / Scout):** MCP wird in Studio Code Server (HA Add-on) genutzt, um direkt aus dem Edge-Compute-Layer System-Kontexte an Cursor/MTHO zu streamen.
...
graph TD
    ...
    SC -->|Vorfilter| DR_CHK{4D_RESONATOR (MTHO_CORE)?}
    DR_CHK -- ON --> DR[4D_RESONATOR (MTHO_CORE): RTX3050]
