...
async def handle_deep_reasoning(payload: dict) -> dict:
    ...
    try:
        ...
        reply = atlas_llm.invoke_heavy_reasoning(
            "Du bist ATLAS. Antworte praezise und analytisch.",
            text
        )
...
async def handle_failover(payload: dict) -> dict:
    ...
    vps_url = (os.getenv("ATLAS_VPS_URL") or "").strip().rstrip("/")
    if not vps_url:
        return {"error": "ATLAS_VPS_URL nicht konfiguriert", "success": False}
