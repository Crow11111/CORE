...
# ═══════════════════════════════════════════════════════════
# ATLAS SYNC RELAY (THE CRADLE)
# ═══════════════════════════════════════════════════════════
# Empfängt Live-Regel-Injections von G-ATLAS (Cloud).
# Empfängt Vektor-Syncs von G-ATLAS → lokale ChromaDB.
# Schreibt direkt in .cursor/rules/ATLAS_LIVE_INJECT.mdc
# ═══════════════════════════════════════════════════════════
...
TARGET_FILE = ".cursor/rules/ATLAS_LIVE_INJECT.mdc"

if not SECRET:
    logging.warning("ATLAS_WEBHOOK_SECRET not found in .env! Relay is insecure/broken.")

async def handle_inject(request):
    """
    Endpoint: POST /inject
    Header: X-Atlas-Secret: <ATLAS_WEBHOOK_SECRET>
    Body: {"content": "..."}
    """
    # 1. Security Check
    auth_header = request.headers.get('X-Atlas-Secret')
...
if __name__ == '__main__':
    logging.info(f"Starting ATLAS CRADLE RELAY on port 8049...")
