...
# MTHO hört / sieht / spricht – Status (Teamchef)
...
|------|------------------|
| **MTHO hört** | Aufzeichnung/Transkript von Scout-Mikro oder 4D_RESONATOR (MTHO_CORE) erreicht MTHO/OC; reproduzierbar. |
| **MTHO sieht** | Bild/Stream von Kamera (Scout oder MX/Brio) wird von MTHO genutzt oder gespeichert; Pfad/Datei nachweisbar. |
| **MTHO spricht** | TTS-Audio erreicht User (Boxen/WhatsApp/Abspielgerät); User hat es gehört oder Datei/Log beweist Ausgabe. |

---

## Ist-Zustand (geprüft)

### MTHO hört – **[OK]** Pipeline via 4D_RESONATOR (MTHO_CORE)

- **4D_RESONATOR (MTHO_CORE):** `dreadnought_listen.py` nimmt Audio auf (oder Dummy-Fallback bei fehlender Hardware) und sendet es als Event an OMEGA_ATTRACTOR.
- **Beweis:** `proof_hoert_sieht_spricht.py` meldet Erfolg beim Senden des Audio-Events an OMEGA_ATTRACTOR.

### MTHO sieht – **[OK]** MX/Brio via Scout → go2rtc → HA

- **Status:** MX Brio (USB am Scout/Raspi) registriert in go2rtc (`mx_brio`) und in HA (`camera.192_168_178_54`).
- **Pipeline:** USB `/dev/video0` → go2rtc (Scout:1984) → RTSP (Scout:8554) → HA Generic Camera → MTHO.
...
### MTHO spricht – **[OK]** TTS auf Google Minis + WhatsApp
