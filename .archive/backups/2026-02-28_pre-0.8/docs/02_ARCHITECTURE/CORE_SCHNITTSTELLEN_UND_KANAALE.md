...
- **Lokal:** `CHROMA_HOST` leer → `CHROMA_LOCAL_PATH` (z. B. `/OMEGA_CORE\data\chroma_db`).
...
- **Collections (laut ARCHITECTURE):** `events`, `insights`; zusätzlich bestehend: `shell_knowledge_graph`, `core_brain_registr` (siehe chroma_client.py).
