...
# ATLAS INFRASTRUCTURE MASTER PLAN
...
- [MX KAMERA ATLAS HOEREN SEHEN](#mx-kamera-atlas-hoeren-sehen)
...
# 00 ATLAS INFRASTRUCTURE MASTER
...
Speicherort auf Dreadnought: `c:\MTHO_CORE\data\openwakeword_models\`
...
| MtH11 | 192.168.178.20 | 18:C0:4D:DB:2D:B0 | Haupt-PC (Dreadnought) |
...
## MX KAMERA ATLAS HOEREN SEHEN

### MX-Kamera (Logitech Brio) – ATLAS hören und sehen

Kurzanleitung: So nutzt ATLAS die MX/Brio am Dreadnought-PC zum **Sehen** und (optional) zum **Hören**.  
...
1. **HA-Seite:** USB-MX am Scout in Home Assistant sichtbar machen.
   ...
   - **Entity-ID:** z.B. `camera.scout_mx` (frei wählbar; Default in ATLAS: `camera.scout_mx`).
   - **Anlegen:** YAML (`configuration.yaml`) oder **Einstellungen → Geräte & Dienste → Kamera hinzufügen** (FFmpeg/Generic Camera).
2. **ATLAS-Seite:** `SCOUT_MX_SNAPSHOT_URL` setzen = `HASS_URL` + `/api/camera_proxy/<entity_id>`.
...
- Skript: `python -m src.scripts.dreadnought_listen` – Aufnahme → WAV → Event an OC Brain.
- Unter Windows das gewünschte Mikrofon als **Standard-Eingabegerät** setzen, dann braucht ATLAS keine weitere Konfiguration.
...
- **Dreadnought (ATLAS API):** 192.168.178.20, Port 8000
...
4. **ATLAS API URL:** z.B. `http://192.168.178.20:8000`
...
### ATLAS Voice Pipeline – Audio-Input Durchstich

**Problem:** Pipeline konfiguriert (Whisper, Piper, openWakeWord, ATLAS Conversation), aber **kein Audio-INPUT** erreicht die Assist Pipeline. Brio-Audio ist in go2rtc sichtbar, aber go2rtc liefert nicht an Assist.
...
```bash
### Via HA Supervisor API (von Dreadnought, mit HASS_TOKEN):
...
3. Erwartung: Whisper transkribiert → ATLAS Conversation → Piper spricht Antwort
...
##### Fallback C: Dreadnought Voice Satellite (bereits vorhanden)

`src/voice/dreadnought_voice_satellite.py` – nutzt Mikro **am PC**, nicht am Scout.

- **Nicht** für Scout-Brio geeignet
- Nur wenn Voice-Input vom Dreadnought-PC gewünscht ist
...
- **Dreadnought:** Kern-LLM, Chroma-Option lokal, Cursor/Cloud Agents, alle Keys und sensiblen Daten.  
...
|-----------|------|
| VPS | 187.77.68.250 |
| SSH Key | `c:\MTHO_CORE\.ssh\id_ed25519_hostinger` |
| User | root |
| Port | 8001 |
...
```powershell
### .env: VPS_HOST=187.77.68.250, VPS_USER=root, VPS_SSH_KEY=c:\MTHO_CORE\.ssh\id_ed25519_hostinger
...
ssh -i c:\MTHO_CORE\.ssh\id_ed25519_hostinger root@187.77.68.250 "echo OK"
