...
# MTHO WUJI – 3-Hop-Kommunikationskette Validierung
...
|------|-------------------------------------|------|--------|
| **WhatsApp → MTHO → Response** | 6 (WA→HA→rest_cmd→MTHO→Auth→Triage→LLM/HA→HA→WA) | ≤3 | ⚠️ REDESIGN |
| **HA (Scout) → MTHO → Action → HA** | 5 (HA→MTHO→Auth→Triage→HA/OC→HA) | ≤3 | ⚠️ REDESIGN |
| **HA Scout-Direct (Command)** | 4 (HA→MTHO→Auth→Triage→HA) | ≤3 | ⚠️ REDESIGN |
| **HA Scout-Direct (Deep Reasoning)** | 6 (HA→MTHO→Auth→Triage→OMEGA_ATTRACTOR→Response) | ≤3 | ⚠️ REDESIGN |
...
| **OMEGA_ATTRACTOR → MTHO (Webhook-Push)** | 3 (OC→MTHO API→Auth→File) | ≤3 | ✅ OK |
| **MTHO → OMEGA_ATTRACTOR (send)** | 3 (MTHO→OC Gateway→Agent) | ≤3 | ✅ OK |

---

## 2. Detaillierte Hop-Zählung pro Pfad

### 2.1 WhatsApp → MTHO API → Response

```
[1] WhatsApp (User) → HA Addon (Event)
[2] HA Addon → rest_command.atlas_whatsapp_webhook
[3] rest_command → MTHO POST /webhook/whatsapp
...
**Logische Service-Hops:** 6 (HA, rest_cmd, MTHO, Auth, Triage/LLM, HA)  
**Physisch:** WA↔HA↔MTHO (2 Netzwerk-Sprünge)

---

### 2.2 HA (Scout) → MTHO API → Action Dispatch → HA

```
[1] HA Companion App → MTHO POST /webhook/ha_action
...
- WhatsApp Addon → **direkt** MTHO API (ohne HA rest_command)
- Voraussetzung: MTHO-URL von Scout/HA-Netz aus erreichbar; Addon unterstützt custom Webhook-URL
- Hop-Kette: `WhatsApp Addon → MTHO API → [Triage+LLM+HA in einem] → HA send_whatsapp`
- **Ergebnis:** 3 Hops (Addon→MTHO, MTHO intern, MTHO→HA)

**Redesign B – HA als einziger Edge:**
- rest_command + Automation als **ein** logischer Hop („HA Edge“)
- MTHO konsolidiert: Auth + Triage + Action in **einem** Request-Handler (kein separates Triage-Service-Call)
- Hop-Kette: `HA Edge → MTHO (Monolith) → HA Output`
- **Ergebnis:** 3 Hops

**Maßnahme:**  
- `whatsapp_webhook.py`: Triage als Inline-Call (kein extra Service), Auth als Depends (kein Hop)  
- Zählung: HA(1) → MTHO(2) → HA(3) = 3 Hops ✓

---

### 3.2 HA (Scout) → MTHO → Action
...
- Hop-Kette: `HA → MTHO (Auth+Triage+Action) → HA/OC`
- **Ergebnis:** 3 Hops (HA, MTHO, HA/OC)
...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                    MTHO WUJI – 3-HOP-MAXIMUM ARCHITEKTUR                        │
└─────────────────────────────────────────────────────────────────────────────────┘
...
HOP-ZÄHLUNG (pro Pfad):
  HOP 1: Edge (WhatsApp/HA/Cursor) → MTHO API
  HOP 2: MTHO API → Ring-0 Processor (Triage+Validation)
...
|------|--------------|---------------|----------|
| WhatsApp | 6 | 3 | HA als Edge; MTHO Monolith (Auth+Triage+Action) |
