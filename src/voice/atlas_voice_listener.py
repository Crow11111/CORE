...
"""
ATLAS Voice Listener - Continuous voice input for Windows.
...
Usage:
  python -m src.voice.atlas_voice_listener
...
WAKE_WORDS = ["computer", "atlas", "hey atlas"]


class AtlasVoiceListener:
        ...
        while True:
            try:
                ...
                if event_type == "intent-end":
                    atlas_reply = event.get("data", {}).get("intent_output", {}).get("response", {}).get("speech", {}).get("plain", {}).get("speech", "")
                    print(f"[ATLAS] {atlas_reply}")
    ...
    async def listen_continuous(self):
        ...
        """
        print("\n" + "="*50)
        print("ATLAS Voice Listener - Push-to-Talk Mode")
        print("="*50)
        print(f"Microphone: {sd.query_devices(kind='input')['name']}")
        print(f"Pipeline: ATLAS ({PIPELINE_ID[:8]}...)")
        print("="*50)
