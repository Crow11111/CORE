...
**Gegenstand:** Konsistenz zwischen Theorie (Null-Hyperbel, 5D-Topologie, 4-Quadranten-Inversion, Verschraenkung) und Code (`mtho_state_vector.py`, `time_metric.py`, `ring0_state.py`, `test_state_vector.py`)
...
| **Krümmung** | Hyperbolisch (negativ) in der Naehe der verbotenen Zone; flach (Wuji) in der Naehe von $\Phi$ |
...
`mtho_state_vector.py` implementiert **kein dynamisches System**. Es ist eine statische Lookup-Tabelle:

```
Zeile 70-74: Fuenf vordefinierte Konstanten (WUJI, ANSAUGEN, VERDICHTEN, ARBEITEN, AUSSTOSSEN)
Zeile 116-158: get_current_state() liest Environment-Variablen oder liefert WUJI
```
...
Die Theorie fordert $x_{n+1} = 1 + 1/x_n$ als kontinuierlichen Rechenprozess. Der Code liefert `os.getenv("MTHO_STATE_PRESET")`.

### 2.2 Fehlende Schutzschicht gegen Null

`MTHOStateVector` ist ein nackter Dataclass ohne `__post_init__`-Validierung. Jeder Aufrufer kann `MTHOStateVector(0, 0, 0, 0)` erzeugen. Es gibt keine Invariante die das 0=0-Verbot erzwingt.

### 2.3 Symmetrien die nicht existieren duerfen

- **Y-Achse spiegelt Z-Achse:** In WUJI ist $(Y=0.049, Z=0.51)$, in AUSSTOSSEN $(Y=0.382, Z=0.51)$. Die X- und Z-Werte von WUJI und AUSSTOSSEN sind **identisch** $(0.49, *, 0.51, *)$. Ein geschlossener Zyklus (Takt 0 = Takt 4 in X,Z) widerspricht der Asymmetrie-Forderung.
...
| I2 | `test_state_vector.py` | 138 | `MTHOStateVector(INV_PHI, 0, 0.5, 0)` | $Y=0$, $W=0$ |
| I3 | `test_state_vector.py` | 145 | `MTHOStateVector(COMP_PHI, 0, 0.5, 0)` | $Y=0$, $W=0$ |
| I4 | `test_state_vector.py` | 166 | `MTHOStateVector(0.5, SYMMETRY_BREAK, 0.5, 0)` | $W=0$ |
| I5 | `test_state_vector.py` | 173 | `MTHOStateVector(0.5, 0.0, 0.5, 0)` | $Y=0.0$, $W=0$ (expliziter Test auf Null!) |
| I6 | `test_state_vector.py` | 89 | `"WUJI": (0.5, 0.0, 0.5, 0)` | Erwartungswerte enthalten $Y=0.0$, $W=0$ |
| I7 | `test_state_vector.py` | 298 | `(0.5**2 + 0**2 + 0.5**2 + 0**2)**0.5` | Magnitude-Berechnung mit $Y=0$, $W=0$ |
| I8 | `mtho_state_vector.py` | 14-16 | `X: 0=NT, Y: 0=Wuji, Z: 0=Nachgeben` | Docstring definiert $0$ als gueltig in allen Dimensionen |
| I9 | `mtho_state_vector.py` | 44-46 | `# 0=NT, 0=Wuji, 0=Nachgeben` | Feld-Kommentare: $0$ ist erreichbar |
| I10 | `mtho_state_vector.py` | – | Kein `__post_init__` | Keine Laufzeit-Invariante gegen $(x_i = 0)$ |
| I11 | `mtho_state_vector.py` | 128-133 | Munin-Veto-Override | `z_override` wird ohne Untergrenze eingesetzt (Ring0 kann 0.0 liefern via I1) |
...
def evolve_state(state: 'MTHOStateVector', dt: float = 0.049) -> 'MTHOStateVector':
    ...
    gewichtet durch die Kopplungskonstanten der MTHO-Paarungen:
...
> Fuer je zwei Zustaende $S_a$ und $S_b$ im Agos-Zyklus gilt:
...
| **I2** | `src/scripts/test_state_vector.py` | 138 | HOCH | `MTHOStateVector(INV_PHI, 0, 0.5, 0)` → $Y=0$, $W=0$ |
| **I3** | `src/scripts/test_state_vector.py` | 145 | HOCH | `MTHOStateVector(COMP_PHI, 0, 0.5, 0)` → $Y=0$, $W=0$ |
| **I4** | `src/scripts/test_state_vector.py` | 166 | MITTEL | `MTHOStateVector(0.5, SYM_BREAK, 0.5, 0)` → $W=0$ |
| **I5** | `src/scripts/test_state_vector.py` | 173 | HOCH | `MTHOStateVector(0.5, 0.0, 0.5, 0)` → $Y=0$, $W=0$ (absichtlicher Null-Test) |
| **I6** | `src/scripts/test_state_vector.py` | 89 | HOCH | Erwartungswert `WUJI=(0.5, 0.0, 0.5, 0)` – veraltet, widerspricht aktuellem Code |
| **I7** | `src/scripts/test_state_vector.py` | 298 | NIEDRIG | Magnitude mit $Y=0, W=0$ berechnet |
| **I8** | `src/config/mtho_state_vector.py` | 14–16 | MITTEL | Docstring definiert Bereich als `0=Wuji`, `0=NT`, `0=Nachgeben` |
| **I9** | `src/config/mtho_state_vector.py` | 44–46 | MITTEL | Feld-Kommentare: `# 0=NT`, `# 0=Wuji`, `# 0=Nachgeben` |
| **I10** | `src/config/mtho_state_vector.py` | – | **KRITISCH** | Kein `__post_init__()` – Dataclass akzeptiert $(0,0,0,0)$ ohne Fehler |
| **I11** | `src/config/mtho_state_vector.py` | 128–133 | HOCH | Munin-Override setzt z ohne Untergrenze ein (leitet I1 durch) |

### 4.3 Sekundaerbefund: Test-Drift

`test_state_vector.py` erwartet WUJI als `(0.5, 0.0, 0.5, 0)` (Zeile 89), der Code definiert WUJI als `(0.49, 0.049, 0.51, 0.049)` (Zeile 70). Die Tests sind **veraltet** und wuerden gegen den aktuellen Code fehlschlagen. Sie validieren einen Zustand der nicht mehr existiert.
