...
VPS_HOST = os.getenv("VPS_HOST", "").strip() or "atlas-vps"
VPS_USER = os.getenv("VPS_USER", "root").strip()
VPS_DEPLOY_PATH = os.getenv("VPS_DEPLOY_PATH", "/opt/atlas-core").strip()
...
def main() -> None:
    ...
    cmd = (
        ...
        "docker build -f docker/agi-state/Dockerfile.vps -t atlas-vps-slim . && "
        "docker stop atlas-vps-slim 2>/dev/null || true && "
        "docker rm atlas-vps-slim 2>/dev/null || true && "
        "docker run -d --name atlas-vps-slim --env-file .env -p 8001:8001 atlas-vps-slim"
    )
    r = _run(_ssh_base() + [cmd], timeout=300)
    _check(r, "SSH docker build/run fehlgeschlagen")
    logger.success("Deploy abgeschlossen: atlas-vps-slim auf Port 8001")
