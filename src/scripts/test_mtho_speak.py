...
"""
MTHO Sprechen testen: TTS (ElevenLabs) und/oder WhatsApp via HA (Scout).
...
DEFAULT_MESSAGE = "MTHO Prototyp 0.5. Scout wieder online. Sehen, Hoeren, Sprechen – Test."


def main():
    msg = os.getenv("MTHO_SPEAK_MESSAGE", DEFAULT_MESSAGE)
    out = []
    ...
    if os.getenv("ELEVENLABS_API_KEY"):
        try:
            ...
            path = speak_text(msg, role_name="mtho_dialog", output_path=None, play=False)
            if path:
    ...
    if target and os.getenv("HASS_URL") and os.getenv("HASS_TOKEN"):
        try:
            ...
            ok = HAClient().send_whatsapp(to_number=target, text=f"[MTHO] {msg}")
