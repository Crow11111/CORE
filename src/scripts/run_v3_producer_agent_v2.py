...
MODEL_NAME = "gemini-1.5-pro-latest" # Use stable model
...
for wp in old_wp_files:
    ...
    if wp_path.exists():
        with open(wp_path, "r", encoding="utf-8") as f:
            old_wp_content += f"\n\n--- {wp} ---\n{f.read()[:10000]}" # Limit size

# Load Deep Research Audits and Operator Documents
dr_files = [
...
system_instruction_reports = """Du bist der OMEGA CORE Producer Agent. Deine Aufgabe ist die strikte Analyse, Verifikation und Falsifikation der Kritikpunkte an der FTOE V2.
Du schreibst ZWISCHENBERICHTE für die 4 Hauptkritikpunkte aus dem Masterplan.
Nutze die SOTA 2026 Ergebnisse, die alten Whitepapers und die DeepResearch-Audits/Operator-Dokumente, um die Kritikpunkte zu verifizieren oder zu falsifizieren.
...
DEEP RESEARCH AUDITS & OPERATOR DOKUMENTE (Auszüge):
{dr_content}

AUFGABE:
Erstelle die 4 Zwischenberichte gemäß dem Masterplan. Analysiere jeden Kritikpunkt, verifiziere/falsifiziere ihn anhand der Daten (insbesondere der neuen Operator-Dokumente) und erkläre, wie er in V3 umgesetzt wird.
"""
...
try:
    response_reports = model.generate_content(prompt_reports)
...
system_instruction_v3 = """Du bist der OMEGA CORE Producer Agent. Deine finale Aufgabe ist die Erstellung des Whitepapers V3 (FTOE).
...
Integriere alle Erkenntnisse aus den Zwischenberichten, der SOTA-Forschung und den Operator-Dokumenten.
...
try:
    response_v3 = model_v3.generate_content(prompt_v3)
