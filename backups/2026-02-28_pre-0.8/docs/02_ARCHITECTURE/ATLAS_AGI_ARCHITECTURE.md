...
- **Phase 2 (Software-Refactoring - TDD-Pflicht):** Autonomer Dev-Agent-Spawn. Code-Suche nach algorithmischer Kompression (z.B. GGUF Quantisierung, Loop-Optimierung). **Muss** vor Deployment in einer Sandbox bewiesen werden. Schlägt der Test fehl, wird der Agent mit hartem Feedback ("Beweise es") in die nächste Schleife geschickt.
