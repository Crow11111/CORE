...
res_0 = abs(CrystalGridEngine.apply_operator_query(0.0))
...
res_1 = abs(CrystalGridEngine.apply_operator_query(1.0))
print(f'[2] Input 1.0 -> Snapped to: {res_1} (Erwartet: 1.0)')
# 1.0 ist jetzt kardanisch entkoppelt (1j), abs(1j) = 1.0
if res_1 != 1.0:
    print(f'FEHLER: 1.0 nicht korrekt abgefangen! (Ist: {res_1})')
    sys.exit(1)

# Test 3: Baryonic Delta Snapping
res_delta = abs(CrystalGridEngine.apply_operator_query(0.048))
...
res_sym = abs(CrystalGridEngine.apply_operator_query(0.5))
print(f'[4] Input 0.5 -> Snapped to: {res_sym} (Erwartet: 0.549)')
# Dynamo Push: 0.5 + (0.049 / 2) / 0.5 = 0.5 + 0.049 = 0.549
if abs(res_sym - 0.549) > 0.001:
    print(f'FEHLER: Symmetrie-Bruch defekt! (Ist: {res_sym})')
    sys.exit(1)
