...
Anleitung zum Erstellen eines Custom Wake Words "MTHO" fuer openWakeWord in Home Assistant.
...
| **Pipeline** | MTHO (ID: `01hzktez4kncsm0sr1qx32hy5x`) |
...
1. **Navigation:** Einstellungen > Voice assistants > MTHO > Wake word
...
### 4. Custom Wake Word "MTHO" trainieren
...
Speicherort auf 4D_RESONATOR (MTHO_CORE): `c:\MTHO_CORE\data\openwakeword_models\`
...
| 4 | MTHO Wake Word Samples aufnehmen | Ausstehend |
| 5 | MTHO Custom Training durchfuehren | Ausstehend |
...
*Erstellt: 2026-03-04*
*Pipeline: MTHO (01hzktez4kncsm0sr1qx32hy5x)*
...
Kurzanleitung: So nutzt MTHO die MX/Brio am 4D_RESONATOR (MTHO_CORE)-PC zum **Sehen** und (optional) zum **Hören**.  
...
1. **HA-Seite:** USB-MX am Scout in Home Assistant sichtbar machen.
   ...
   - **Entity-ID:** z.B. `camera.scout_mx` (frei wählbar; Default in MTHO: `camera.scout_mx`).
   - **Anlegen:** YAML (`configuration.yaml`) oder **Einstellungen → Geräte & Dienste → Kamera hinzufügen** (FFmpeg/Generic Camera).
2. **MTHO-Seite:** `SCOUT_MX_SNAPSHOT_URL` setzen = `HASS_URL` + `/api/camera_proxy/<entity_id>`.
...
- Unter Windows das gewünschte Mikrofon als **Standard-Eingabegerät** setzen, dann braucht MTHO keine weitere Konfiguration.
