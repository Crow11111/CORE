...
|------------|--------|--------|
| OpenClaw Admin (OC Brain) | LÄUFT | Z1–Z5; API 18789, Nginx 443 |
| Scout → OC Brain (Event) | LÄUFT | send_event_to_oc_brain, scout_send_event_to_oc |
| Event-Ingest | LÄUFT | POST /api/atlas/event, GET /api/atlas/events, data/events |
| Voice (ElevenLabs) | LÄUFT | POST /api/atlas/tts, GET /api/atlas/voice/roles |
| OC Brain im Task-Loop | LÄUFT | send_message_to_agent; Priorisierung getestet |
| API-Health gebündelt | LÄUFT | GET /api/atlas/status (Gateway, Event-Ingest, Voice-Roles, letzte Event-IDs) |
...
| fetch_oc_submissions Roundtrip | LÄUFT | Z15: OC schreibt, Dreadnought holt |
...
- Hören: Pipeline Scout-Mikro/Assist → ATLAS.
...
- **Scout → OC Brain:** Event nach Neustart oder bei Trigger – `SCOUT_HA_EVENT_AN_OC_BRAIN.md` (rest_command + Automation).
- **ATLAS Sprechen:** `python -m src.scripts.test_atlas_speak` – TTS (ElevenLabs) und/oder WhatsApp via HA. TTS getestet (OK); WhatsApp sobald Scout/HA wieder erreichbar.
