...
**Sektion 4: CORE Presence Director (Robuste Anwesenheitserkennung)**

**4.1 Funktionsbeschreibung**

Der CORE Presence Director ist die Kernmetrik zur Bestimmung der physischen Anwesenheit von Administratoren/Usern im Gebäude. Um die Fehlerhaftigkeit einzelner Sensoren (z.B. iPhone Ping-Abbrüche, WLAN-Standby) zu eliminieren, verlässt sich CORE nicht auf binäre Zustände, sondern nutzt ein probabilistisches Modell (Bayesian Sensor) in Home Assistant.
...
Sobald der Bayesian Sensor den Schwellenwert von >85% überschreitet oder unterschreitet (mit einem 5-minütigen Puffer gegen Netzwerkschwankungen), greift der "CORE Presence Director" (Home Assistant Automation). 
