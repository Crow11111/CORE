...
"""
MTHO Voice Listener - Continuous voice input for Windows.
...
Usage:
  python -m src.voice.mtho_voice_listener
...
WAKE_WORDS = ["computer", "mtho", "hey mtho"]
...
class MthoVoiceListener:
        ...
        while True:
            try:
                ...
                if event_type == "intent-end":
                    mtho_reply = event.get("data", {}).get("intent_output", {}).get("response", {}).get("speech", {}).get("plain", {}).get("speech", "")
                    print(f"[MTHO] {mtho_reply}")
    ...
    async def listen_continuous(self):
        ...
        """
        print("\n" + "="*50)
        print("MTHO Voice Listener - Push-to-Talk Mode")
        print("="*50)
        print(f"Microphone: {sd.query_devices(kind='input')['name']}")
        print(f"Pipeline: MTHO ({PIPELINE_ID[:8]}...)")
        print("="*50)
